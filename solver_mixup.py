...
class Solver(object):
    def __init__(self, config):
        ...
        self.consistency_index_dict, self.inconsistency_index_dict = {}, {}
        self.epoch_num = 0
    ...
    def train_onestep(self, epoch, record_file_path=None):
        ...
        # TODO 研究点：重新设计 dataloader_target_resample， 采用样本 id 信息 —> DONE
        dataloader_target_resample = inconsistency_target_sampler.re_sample_inconsistency_target(
            list(self.inconsistency_index_dict.keys()), self.dataloader_target_train, self.batch_size)
        ...
        for batch_idx in range(iteration_total):
            ...
            if batch_idx % iteration_target == 0:
                predict_dict_record_file_path = record_file_path.split('.txt')[0] + '_predict_info.txt'
                util.record_log(predict_dict_record_file_path, self.consistency_index_dict.__str__() + '\n')
                util.record_log(predict_dict_record_file_path, self.inconsistency_index_dict.__str__() + '\n')
                self.consistency_index_dict.clear()
                self.inconsistency_index_dict.clear()
            ...
            index_t, img_t, label_t_invisible = iter_dataloader_target_train.next()
            (index_r, img_r, label_r_invisible) = iter_dataloader_target_resample.next() if iteration_resample else (None, None, None)
            if iteration_resample == 0 or img_r.shape[0] == 1:
                resample_consideration = False

            index_s, index_t = index_s.cuda(), index_t.cuda()
            ...
            predict_c1, predict_c2 = output_t1.max(1)[1], output_t2.max(1)[1]
            self.consistency_index_dict, self.inconsistency_index_dict = util.filter_data_index(
                index_list=index_t, real_label=label_t_invisible, predict_c1=predict_c1, predict_c2=predict_c2)

            # TODO 研究点：自定义 discrepancy loss, e.g., symmetric_mse_loss.
            loss_discrepancy = util.loss_discrepancy(output_t1, output_t2)

            loss_consistency_output_t1, loss_consistency_output_t2 = 0.0, 0.0
            ...
            if batch_idx % self.batch_interval == 0:
                ...
                train_info = '\tGRL_coefficient:{:.6f}' \
                             '\tConsistency weight:{:.6f}\tConsistency loss_t1:{:.6f}\tConsistency loss_t2:{:.6f}' \
                             '\tDiscrepancy_Resample Weight:{:.6f}\tResample Discrepancy loss{:.6f}' \
                             '\tResample Pool Num:{}' \
                    .format(grl_coeff,
                            weight_consistency.data.item(),
